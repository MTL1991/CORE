...
app.configure(function(){
 ...
 //Middleware para cerrar sesión automáticamente pasado un minuto

app.use(function(req, res, next)  {
  if(req.session.user != undefined){
    var tiempo = new Date ();
    //console.log ('¿Now? ' + tiempo.getTime());         
    if (req.session.user.tiempoLimite == undefined){
      tiempo = tiempo.getTime() + 60000;
      req.session.user.tiempoLimite = tiempo;
     // console.log ('TL ' + req.session.user.tiempoLimite);
    }
    else{
      if ((tiempo.getTime() - req.session.user.tiempoLimite) >= 0){
        delete req.session.user;
        req.flash('info', 'Sesión finalizada por inactividad');
      }
      else{
        tiempo = tiempo.getTime() + 60000;
        req.session.user.tiempoLimite = tiempo;
